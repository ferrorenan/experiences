...
function scrollTop() {
  $(window).scroll(function () {
    ...
    $('.js-bol').css({
      'transform': 'translate(-50%, -50%) rotate(' + x + 'deg)'
    });
    console.log(x + 'px');
  });
}
...
$(function () {
  ...
  if (document.getElementById('custom-container').addEventListener) {
    ...
    document.getElementById('custom-container').addEventListener("mousewheel", scrollHorizontally, false); // Firefox
  ...
  scrollTop(); //setupSideDrawer();
});
window.addEventListener('load', function () {// setupInfiniteScroll();
});
